...
async function updateFavouriteIcons() {
  ...
  try {
    // First, check the auth status to avoid making a failing request for guests.
...
async function handleAddToFavourites(productId, productName) {
  ...
  // First, check the user's login status to decide the strategy.
  const authRes = await fetch(`${API_BASE_URL}/api/auth/status`, { credentials: 'include' });
  const authState = await authRes.json();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function handleRemoveFromFavourites(productId) {
    ...
    try {
      ...
      // We don't need to strictly handle the response unless we want to show an error.
  ...
  function renderSlideshow(container, title, products) {
    ...
    products.forEach(p => {
      // This structure is similar to the one in product.js for related products
    ...
    // We can move this function to a shared utility file later. For now, let's include product.js
    if (typeof setupRelatedProductSlideshow === 'function') {
      setupRelatedProductSlideshow(); // The function now finds its own container
    }
  }
